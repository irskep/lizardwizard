...
bezier_coefficients = []
def bezier(x1a, y1a, x1b, y1b, x2b, y2b, x2a, y2a, resolution=20):
    coords = []
    if not bezier_coefficients:
        for i in xrange(resolution+1):
            ...
            t2 = 3 * t ** 2 * (1 - t)
            t3 = t ** 3
            bezier_coefficients.append([t0, t1, t2, t3])
    for i, t in enumerate(bezier_coefficients):
        px = t[0] * x1a + t[1] * x1b + t[2] * x2b + t[3] * x2a
        py = t[0] * y1a + t[1] * y1b + t[2] * y2b + t[3] * y2a
        coords.extend((px, py))
    pyglet.graphics.draw(len(coords)/2, pyglet.gl.GL_LINE_STRIP, ('v2f', coords))
